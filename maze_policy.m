    ...
    for i = 1:(s*s)
            for j = 1:(s*s)
                 if any(goal == j)
                     ...
                     if B(j,policy(j))
                     value(j) = reward(j,policy_ac(policy(j)),goal,s) + gama*value(sigma(j,policy_ac(policy(j)),s));
                     end
                 end
            end
            if any(goal == i)
