...
maxiter = 400000;
g = 0;
h = 1;%an index for determine if first state in first run is goal or not
ka = 10;
kaa = zeros(4,1);
aa =0;
...
%     end
    if(any(current_state == goal))
        ...
        for l = 1 : 4
           aa = aa + ka^Q_table((current_state-1)*4+l);%makhraj kasr
        end
        for l = 1:4
            kaa(l) = ka^Q_table((current_state-1)*4+l)/aa;%probability of each action
        end
        action =(2*eps)*floor(rand/(2*eps));
        aa = 0;

        if (0 <= action) && (action < kaa(1))
            action = 1;
        elseif (kaa(1) <= action) && (action < (kaa(1) + kaa(2)))
            action = 2;
        elseif ((kaa(1) + kaa(2)) <=action) && (action < (kaa(1) + kaa(2) + kaa(3)))
            action = 3;
        else 
            action = 4;
        end
        ...
        if(B(current_state,action))
                    w(1,h) = ((current_state - 1)*4) + action;%index for array for updatin by each episode
                    w(2,h) = reward(current_state,policy_ac(action),goal,s) + ...%value of array for updatin by each episode
